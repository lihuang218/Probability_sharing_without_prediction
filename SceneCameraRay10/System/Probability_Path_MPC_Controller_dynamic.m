function [sys,x0,str,ts] = Probability_Path_MPC_Controller_dynamic(t,x,u,flag)
...
global VehiclePara; 
    VehiclePara.m   = 1351;   %m为车辆质量,Kg; Sprung mass = 1370
    VehiclePara.g   = 9.8;
    VehiclePara.Lf  = 1.232;  % 1.05
    VehiclePara.Lr  = 1.468;  % 1.55
    VehiclePara.L   = 2.7;  %VehiclePara.Lf + VehiclePara.Lr;
...
%     x_start=zeros(Nc+1,1);%加入一个起始点
    ...
    % 使用casadi求解
    % 构建优化问题
    import casadi.*  % 导入casadi
    n = 6;
    x = MX.sym('x',n);
    objective = 0.5*x'*H*x + f'*x; % 定义目标函数
    constraints = {lb <= x <= ub, A_cons*x <= b_cons}; % 添加约束
    % 创建优化器
    opts = struct;
    opts.ipopt.print_level = 0;
    nlp = struct('x',x,'f',objective,'g',vertcat(constraints{:}));
